...
useEffect(() => {
  ...
  checkIfInWatchlist(user?.uid, id)
  .then((data) => {
...
return (
    <Box>
            ...
            <Container maxW={"container.xl"}>
                <Flex alignItems={"center"} gap="10" flexDirection={{base: "column", md: "row"}}>
                    ...
                    <Box>
                            ...
                            {type === "movie" && (
                                <>
                                    <Flex alignItems={"center"}>
                                        ...
                                        <Text fontSize={"small"}>{minutesToHours(details?.runtime)}</Text>
                                    </Flex>
                                </>
                            )}
                        </Flex>
                        ...
                        <Flex alignItems={"center"} gap={"4"}>
                            ...
                            >
                                <CircularProgressLabel fontSize={"lg"}>
                                    ...
                                    <Box as="span" fontSize={"small"}>%</Box>
                        ...
                        <Text color={"gray.400"} fontSize={"sm"} fontStyle={"italic"} my={"5"}>{details?.tagline}</Text>
                        <Heading fontSize={"xl"} mb={"3"}>Overview</Heading>
                        <Text fontSize={"md"} mb={"3"}>{details?.overview}</Text>
                        <Flex mt="6" gap="2">
                            {details?.genres?.map((genre) => (
                                <Badge key={genre?.key} h={6} alignContent={"center"}>{genre?.name}</Badge>
                            ))}
                        </Flex>
                    </Box>
                </Flex>
        ...
        <Container maxW={"container.xl"} pb={"10"}>
            <Heading as={"h2"} fontSize={"md"} textTransform={"uppercase"} mt={"10"} mb={"5"}>Videos</Heading>
            ...
            <Flex mt={"5"} mb={"10"} overflowX={"scroll"} gap={"5"}>
                {cast?.length === 0 && <Text>No Details Found</Text>}
                {cast && cast?.map((item) => (
